...
def process_csv(uploaded_file, client_id=None, ip=None):
    ...
    if results:
        ...
        export_format = st.radio(
            ...
            ["CSV", "Excel (XLSX)", "JSON", "Text (TXT)"],
            horizontal=True,
            key="csv_export_format"
        )
        ...
        with col_dl1:
            if export_format == "CSV":
                ...
                st.download_button(
                    "⬇️ Download Full Details (Excel)",
                    buffer.getvalue(),
                    "verified_full_details.xlsx",
                    "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    key="full_excel",
                    use_container_width=True
                )
            elif export_format == "JSON":
                json_full = result_df.to_json(orient='records', indent=2).encode("utf-8")
                st.download_button(
                    "⬇️ Download Full Details (JSON)",
                    json_full,
                    "verified_full_details.json",
                    "application/json",
                    key="full_json",
                    use_container_width=True
                )
            elif export_format == "Text (TXT)":
                txt_full = result_df.to_csv(index=False, sep='\t').encode("utf-8")
                st.download_button(
                    "⬇️ Download Full Details (TXT)",
                    txt_full,
                    "verified_full_details.txt",
                    "text/plain",
                    key="full_txt",
                    use_container_width=True
                )
        ...
        with col_dl2:
            if export_format == "CSV":
                ...
                buffer = BytesIO()
                with pd.ExcelWriter(buffer, engine='openpyxl') as writer:
                    separated_df.to_excel(writer, sheet_name='Separated', index=False)
                st.download_button(
                    "⬇️ Download Separated (Excel)",
                    buffer.getvalue(),
                    "verified_separated.xlsx",
                    "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    key="separated_excel",
                    use_container_width=True
                )
            elif export_format == "JSON":
                json_separated = separated_df.to_json(orient='records', indent=2).encode("utf-8")
                st.download_button(
                    "⬇️ Download Separated (JSON)",
                    json_separated,
                    "verified_separated.json",
                    "application/json",
                    key="separated_json",
                    use_container_width=True
                )
            elif export_format == "Text (TXT)":
                txt_separated = separated_df.to_csv(index=False, sep='\t').encode("utf-8")
                st.download_button(
                    "⬇️ Download Separated (TXT)",
                    txt_separated,
                    "verified_separated.txt",
                    "text/plain",
                    key="separated_txt",
                    use_container_width=True
                )
    ...
    if verify_button:
        ...
        if results:
            ...
            export_format = st.radio(
                ...
                ["CSV", "Excel (XLSX)", "JSON", "Text (TXT)"],
                horizontal=True,
                key=f"export_format_{client_id}"
            )
            
            col_dl1, col_dl2 = st.columns(2)
            
            with col_dl1:
                st.markdown("#### 📄 Full Details Export")
                ...
                if export_format == "CSV":
                    ...
                    txt_full = result_df.to_csv(index=False, sep='\t').encode("utf-8")
                    st.download_button(
                        "⬇️ Download Full Results (TXT)",
                        txt_full,
                        "verified_full_details.txt",
                        "text/plain",
                        key=f"full_txt_{client_id}",
                        use_container_width=True
                    )
            
            with col_dl2:
                st.markdown("#### 📊 Separated Export (Valid/Risky/Invalid)")
                ...
                if export_format == "CSV":
                    ...
                    buffer = BytesIO()
                    with pd.ExcelWriter(buffer, engine='openpyxl') as writer:
                        separated_df.to_excel(writer, sheet_name='Separated Results', index=False)
                        # Color coding
                        worksheet = writer.sheets['Separated Results']
                        from openpyxl.styles import PatternFill
                        
                        # Valid column (green)
                        for row in range(2, len(separated_df) + 2):
                        ...
                        # Auto-adjust widths
                        for idx in range(len(separated_df.columns)):
                            worksheet.column_dimensions[chr(65 + idx)].width = 35
                    ...
                    st.download_button(
                        ...
                        "verified_separated.xlsx",
                        "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                        key=f"separated_excel_{client_id}",
                        use_container_width=True
                    )
                    ...
                    st.download_button(
                        ...
                        "verified_separated.json",
                        "application/json",
                        key=f"separated_json_{client_id}",
                        use_container_width=True
                    )
                    ...
                    st.download_button(
                        ...
                        "verified_separated.txt",
                        "text/plain",
                        key=f"separated_txt_{client_id}",
                        use_container_width=True
                    )
